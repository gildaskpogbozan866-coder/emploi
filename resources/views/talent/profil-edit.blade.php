...
@section('title', 'Modifier mon profil Talent')
...
<div class="dash-content">
  <div class="dash-content__header">
    <h1 class="dash-content__title">Modifier mon profil</h1>
    <p style="color:#6b7a8d;margin:0">Mettez à jour vos informations professionnelles</p>
  </div>

  <div style="background:#fff;border:1px solid #e2e8f0;border-radius:14px;padding:28px 32px;max-width:680px">
    <form method="POST" action="{{ route('talent.profil.update') }}" enctype="multipart/form-data">
      @csrf @method('PUT')
      ...
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:16px;margin-bottom:16px">
        <div>
          ...
          <input type="text" name="metier" value="{{ old('metier', $profil->metier) }}" required
        ...
        <div>
          ...
          <input type="text" name="specialite" value="{{ old('specialite', $profil->specialite) }}"
      ...
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:16px;margin-bottom:16px">
        <div>
          ...
          <input type="text" name="pays" value="{{ old('pays', $profil->pays) }}" required
        ...
        <div>
          ...
          <input type="text" name="ville" value="{{ old('ville', $profil->ville) }}"
                 style="width:100%;padding:10px 14px;border:1.5px solid #d1d5db;border-radius:8px;font-size:14px;box-sizing:border-box">
        </div>
      </div>
      ...
      <div style="margin-bottom:16px">
        ...
        <textarea name="bio" rows="4"
                  style="width:100%;padding:10px 14px;border:1.5px solid #d1d5db;border-radius:8px;font-size:14px;resize:vertical;box-sizing:border-box">{{ old('bio', $profil->bio) }}</textarea>
      </div>

      <div style="margin-bottom:16px">
        <label style="display:block;font-size:13px;font-weight:600;color:#374151;margin-bottom:5px">Compétences</label>
        <input type="text" name="competences" value="{{ old('competences', $profil->competences) }}"
               style="width:100%;padding:10px 14px;border:1.5px solid #d1d5db;border-radius:8px;font-size:14px;box-sizing:border-box">
        <p style="font-size:12px;color:#94a3b8;margin:4px 0 0">Séparées par des virgules</p>
      </div>

      <div style="display:grid;grid-template-columns:1fr 1fr;gap:16px;margin-bottom:16px">
        <div>
          <label style="display:block;font-size:13px;font-weight:600;color:#374151;margin-bottom:5px">Expérience</label>
          <input type="text" name="experience" value="{{ old('experience', $profil->experience) }}"
                 style="width:100%;padding:10px 14px;border:1.5px solid #d1d5db;border-radius:8px;font-size:14px;box-sizing:border-box">
        </div>
        <div>
          <label style="display:block;font-size:13px;font-weight:600;color:#374151;margin-bottom:5px">Langues</label>
          <input type="text" name="langues" value="{{ old('langues', $profil->langues) }}"
                 style="width:100%;padding:10px 14px;border:1.5px solid #d1d5db;border-radius:8px;font-size:14px;box-sizing:border-box">
        </div>
      </div>

      <div style="margin-bottom:24px">
        <label style="display:block;font-size:13px;font-weight:600;color:#374151;margin-bottom:5px">Nouvelle photo de profil</label>
        @if($profil->photo)
          <div style="margin-bottom:8px">
            <img src="{{ asset('storage/'.$profil->photo) }}" alt="Photo actuelle" style="width:64px;height:64px;border-radius:50%;object-fit:cover;border:2px solid #e2e8f0">
            <span style="font-size:12px;color:#64748b;margin-left:8px">Photo actuelle</span>
          </div>
        @endif
      ...
      <div style="display:flex;gap:12px">
        ...
        <a href="{{ route('talent.profil') }}" style="padding:11px 20px;background:#f1f5f9;color:#374151;border-radius:8px;font-weight:600;font-size:14px;text-decoration:none">
          Annuler
        </a>
      </div>
    </form>
