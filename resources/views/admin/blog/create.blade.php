...
.be-img-zone__ico { font-size: 28px; margin-bottom: 6px; }
.be-img-zone__text { font-size: 13px; color: #64748b; line-height: 1.4; }
.be-img-zone__text strong { color: #042C53; }
.be-img-preview {
  width: 100%; height: 150px;
  border-radius: 9px; overflow: hidden;
  background: #f1f5f9; display: none; margin-bottom: 10px;
}
.be-img-preview img { width: 100%; height: 100%; object-fit: cover; }
...
<form method="POST" action="{{ route('admin.blog.store') }}" enctype="multipart/form-data" class="blog-editor">
  @csrf
  <div class="blog-editor__grid">
    ...
    <div class="blog-editor__col">
        ...
        <div class="be-card__body">
          <div class="be-field">
            ...
            <textarea class="be-textarea" id="extrait" name="extrait" rows="3"
                      placeholder="Résumé visible dans la liste des articles (2-3 phrases)…"
                      oninput="updateCount(this,'extrait-count',500)">{{ old('extrait') }}</textarea>
      ...
      <div class="be-card">
        ...
        <div class="be-ai-bar">
          ...
          <span class="be-ai-note">Génère automatiquement le contenu à partir du titre, de la catégorie et de l'extrait</span>
        </div>

        <div style="padding:0">
          <textarea id="summernote-contenu" name="contenu">{{ old('contenu') }}</textarea>
    ...
    <div class="blog-editor__side">
      ...
      <div class="be-card">
        ...
        <div class="be-card__body">
          ...
          <div class="be-field">
            ...
            <div class="be-status-row" id="status-indicator">
              <span class="be-status-dot be-status-dot--draft" id="status-dot"></span>
              <span id="status-label">Brouillon — non visible</span>
            </div>
          </div>
          ...
          <div class="be-field">
            ...
            <div style="display:flex;align-items:center;gap:8px">
              <input class="be-input" type="number" id="temps_lecture" name="temps_lecture"
                     value="{{ old('temps_lecture', 5) }}" min="1" max="60" style="width:80px;flex-shrink:0">
      ...
      <div class="be-card">
        ...
        <div class="be-card__body">
          <div class="be-img-preview" id="img-preview-wrap">
          ...
          <div class="be-img-zone" onclick="document.getElementById('img-input').click()">
            ...
            <div class="be-img-zone__ico">🖼️</div>
            <p class="be-img-zone__text"><strong>Cliquez pour choisir</strong><br>JPG, PNG · max 3 Mo</p>
          </div>
        </div>
      </div>
...
$(function() {
  $('#summernote-contenu').summernote({
    lang: 'fr-FR',
    height: 460,
    placeholder: 'Rédigez votre article ici, ou cliquez sur "Générer avec l\'IA" pour créer le contenu automatiquement…',
    toolbar: [
      ['style',    ['style']],
...
function updateStatusIndicator(sel) {
  ...
  if (!dot || !label) return;
  if (sel.value === 'publie') {
    dot.className = 'be-status-dot be-status-dot--publish';
    label.textContent = 'Publié — visible par tous';
  } else {
    dot.className = 'be-status-dot be-status-dot--draft';
    label.textContent = 'Brouillon — non visible';
  }
}
  ...
  $.ajax({
    ...
    success: function(res) {
      if (res.contenu) {
        ...
        showToast('Contenu généré avec succès !', 'ok');
      } else {
