...
<div class="rec-topbar">
  <div class="rec-topbar__left">
    <a href="{{ route('recruteur.abonnement') }}" class="rec-btn rec-btn--outline" style="margin-bottom:8px;font-size:12px">
      ← Retour à mes abonnements
    </a>
    <h1>Choisir un plan</h1>
    <p>Sélectionnez le plan adapté à vos besoins de recrutement</p>
  </div>
</div>
...
<div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(320px,1fr));gap:20px">
  ...
  <div style="background:#fff;border:2px solid {{ $isActif ? '#185FA5' : '#e2e8f0' }};border-radius:16px;overflow:hidden;display:flex;flex-direction:column;
    {{ !$plan->is_free && !$loop->first ? 'box-shadow:0 4px 28px rgba(24,95,165,.15)' : '' }}">
...
<div style="margin-top:28px;background:#f8fafc;border:1px solid #e2e8f0;border-radius:14px;padding:22px 26px">
  ...
  <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(180px,1fr));gap:14px">
    @foreach([
      ...
      ['3', 'Activé après confirmation', 'Dès validation par notre équipe, vos annonces sont publiables.'],
    ] as [$n, $titre, $desc])
